...
interface FeatureSectionProps {
  ...
  cards: Card[];
  currentSlug?: string;
  relevantSlugs?: string[]; // 👈 Relevant cards ki custom list
}

const FeatureCard = ({ card, index }: { card: Card; index: number }) => {
  return (
    <div className="relative pt-10">
      <Link href={`/services/${card.slug}`} className="block h-[350px]">
        <motion.div
          ...
          border-2 border-[#335ECE] pt-14 pb-6 px-5 text-center
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center">
            <h3 className="text-xl font-bold text-[#335ECE] mb-3 group-hover:text-white transition-colors leading-snug">
              {card.title}
            </h3>
            <div className="w-16 h-[3px] bg-[#335ECE] group-hover:bg-white mb-4 rounded-full transition-colors duration-300" />
            
            <p 
              className="text-[14px] text-center text-[#666666] leading-relaxed group-hover:text-white/80 transition-colors"
              style={{
                display: "-webkit-box",
                WebkitLineClamp: 3,
...
export default function FeatureSection({
  ...
  return (
    <section className="relative py-8 px-2 md:px-6 bg-[#FFFFFF] overflow-hidden">
